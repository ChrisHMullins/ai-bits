...
Two more rules that decide *how* an item is written:
...
- **Say what happens if they don't answer.** Where a sensible default exists and
  is reversible, name it. This turns a blocking ask into a non-blocking one and
  is usually the single most useful line in an item.
...
4. **If you don't answer** — one indented bullet naming the reversible default,
   where one exists. Usually the most useful line in the item.
